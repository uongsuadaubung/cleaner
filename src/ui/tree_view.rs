...
pub fn render_tree_view(ui: &mut egui::Ui, entries: &mut Vec<FileEntry>, sort_state: Option<SortState>) -> Option<SortCriteria> {
    let scroll_bar_width = ui.spacing().scroll.bar_width;
    let total_width = ui.available_width();
    // Trừ đi thanh cuộn và một khoảng lề nhỏ để không bị dính sát
    ...
    egui::ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            for idx in 0..entries.len() {
                render_entry(ui, &mut entries[idx], 0, content_width);
            }
        });
...
/// Render header cột
fn render_header(ui: &mut egui::Ui, width: f32, entries: &mut Vec<FileEntry>, sort_state: Option<SortState>) -> Option<SortCriteria> {
    ...
    ui.horizontal(|ui| {
        ...
        ui.allocate_ui(egui::vec2(CHECKBOX_WIDTH + COLUMN_SPACING + name_width, 20.0), |ui| {
            ui.horizontal(|ui| {
                // Checkbox Select All
                ui.allocate_ui(egui::vec2(CHECKBOX_WIDTH, 20.0), |ui| {
                    ui.centered_and_justified(|ui| {
                        let mut all_selected = !entries.is_empty() && entries.iter().all(|e| e.selected);
                ...
                let label = format!("Tên file / Thư mục{}", get_sort_icon(SortCriteria::Name, sort_state));
                if ui.selectable_label(false, egui::RichText::new(label).strong().color(egui::Color32::from_rgb(79, 195, 247))).clicked() {
        ...
        ui.allocate_ui(egui::vec2(DATE_WIDTH, 20.0), |ui| {
            let label = format!("Ngày tạo{}", get_sort_icon(SortCriteria::Created, sort_state));
            if ui.selectable_label(false, egui::RichText::new(label).strong().color(egui::Color32::from_rgb(79, 195, 247))).clicked() {
        ...
        ui.allocate_ui(egui::vec2(DATE_WIDTH, 20.0), |ui| {
            let label = format!("Ngày sửa{}", get_sort_icon(SortCriteria::Modified, sort_state));
            if ui.selectable_label(false, egui::RichText::new(label).strong().color(egui::Color32::from_rgb(79, 195, 247))).clicked() {
        ...
        ui.allocate_ui(egui::vec2(SIZE_WIDTH, 20.0), |ui| {
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                let label = format!("Dung lượng{}", get_sort_icon(SortCriteria::Size, sort_state));
                if ui.selectable_label(false, egui::RichText::new(label).strong().color(egui::Color32::from_rgb(79, 195, 247))).clicked() {
...
fn render_entry(ui: &mut egui::Ui, entry: &mut FileEntry, depth: usize, width: f32) {
    ...
    // Vẽ 1 hàng (thêm Sense::click để nhận diện double click)
    ...
    // Vẽ nền khi hover (co diện tích lại một chút để không chạm vào đường kẻ phân tách)
    if ui.rect_contains_pointer(rect.shrink2(egui::vec2(0.0, 0.5))) {
        ui.painter().rect_filled(rect, 0.0, egui::Color32::from_white_alpha(15));
    }

    // Xử lý Double Click cho toàn bộ hàng
    ...
    // Đường kẻ phân cách
    ui.painter().hline(rect.left()..=rect.right(), rect.bottom(), ui.visuals().widgets.noninteractive.bg_stroke);

    let start_x = rect.left();
    let row_y = rect.top() + (ROW_HEIGHT - 20.0) / 2.0;

    // 1. Cột Checkbox
    let cb_rect = egui::Rect::from_min_size(egui::pos2(start_x + x_offsets[0], row_y), egui::vec2(CHECKBOX_WIDTH, 20.0));
    ui.allocate_new_ui(egui::UiBuilder::new().max_rect(cb_rect), |ui| {
        ui.centered_and_justified(|ui| {
    ...
    // 2. Cột Tên
    let name_rect = egui::Rect::from_min_size(egui::pos2(start_x + x_offsets[1], row_y), egui::vec2(name_width, 20.0));
    ui.allocate_new_ui(egui::UiBuilder::new().max_rect(name_rect), |ui| {
        ui.horizontal(|ui| {
            ui.spacing_mut().item_spacing.x = 4.0;
            ui.add_space(indent);
            
            // Chỉ hiện cursor pointer khi hover vào vùng Tên
            ...
            if is_dir {
                ...
                ui.add(egui::Label::new(egui::RichText::new(format!("{} {}", icon, name)).strong().color(egui::Color32::from_rgb(255, 238, 88))).selectable(false));
            } else {
                ui.add_space(18.0);
                ui.add(egui::Label::new(egui::RichText::new(format!("{} {}", icon, name)).color(egui::Color32::from_rgb(176, 190, 197))).selectable(false));
            }
        });
    });

    // 3. Cột Ngày tạo
    let c_rect = egui::Rect::from_min_size(egui::pos2(start_x + x_offsets[2], row_y), egui::vec2(DATE_WIDTH, 20.0));
    ui.allocate_new_ui(egui::UiBuilder::new().max_rect(c_rect), |ui| {
        ui.add_space(2.0);
        ui.label(egui::RichText::new(format_date(created)).color(egui::Color32::from_rgb(176, 190, 197)));
    });

    // 4. Cột Ngày sửa
    let m_rect = egui::Rect::from_min_size(egui::pos2(start_x + x_offsets[3], row_y), egui::vec2(DATE_WIDTH, 20.0));
    ui.allocate_new_ui(egui::UiBuilder::new().max_rect(m_rect), |ui| {
        ui.add_space(2.0);
        ui.label(egui::RichText::new(format_date(modified)).color(egui::Color32::from_rgb(176, 190, 197)));
    });

    // 5. Cột Dung lượng
    let s_rect = egui::Rect::from_min_size(egui::pos2(start_x + x_offsets[4], row_y), egui::vec2(SIZE_WIDTH, 20.0));
    ui.allocate_new_ui(egui::UiBuilder::new().max_rect(s_rect), |ui| {
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ui.label(egui::RichText::new(format_size(size)).color(egui::Color32::from_rgb(176, 190, 197)));
        });
    });
